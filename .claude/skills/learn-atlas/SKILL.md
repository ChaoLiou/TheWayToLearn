...
名詞：**waypoint** = 一支影片的資料夾、**route** = 兩站之間的關聯（有型態）、**region** = 主題區。規則在 `rules/atlas.md`。

## 步驟
1. `uv run scripts/atlas.py --status`
...
2. 依 `rules/atlas.md` 更新 `workspace/atlas.json`（格式 `schemas/atlas.schema.json`）：
   ...
   - 沒有關聯就不要硬連。
3. `uv run scripts/atlas.py` → 驗證並產出 `workspace/atlas.html`。
4. 重跑 `uv run scripts/render.py`（不帶參數）讓每站的 `plan.html` 頂部「學習地圖」區塊更新為最新 route。
