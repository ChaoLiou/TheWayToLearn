...
同一份 repo 既可 clone 進來直接用（`.claude/skills` 是 `skills/` 的 symlink），也可當 Claude Code plugin 安裝（plugin 名 `atlas-of-knowledge`，marketplace 名 `thewaytolearn`；指令前綴 `/atlas-of-knowledge:`）。SKILL.md 裡所有指令都寫成 `uv run --project "${CLAUDE_PLUGIN_ROOT:-.}" "${CLAUDE_PLUGIN_ROOT:-.}/scripts/<x>.py"`，兩種模式都能跑。
